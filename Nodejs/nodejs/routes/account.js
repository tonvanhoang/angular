...
router.put('/edit/:id', async function(req, res, next) {
    try {
        const { id } = req.params;
        const { name, role, password, email, avatar } = req.body;
        const hashedPassword = await bcrypt.hash(password, 10);
        const data = await modelsaccount.findByIdAndUpdate(id, { name, role, password: hashedPassword, email, avatar });
...
router.post('/register', async function(req,res,next) {
    try{
        ...
        const {name,role,password,email,avata}= req.body;
        const hashedPassword = await bcrypt.hash(password, 10);
        const data  = await modelsaccount.create({_id,name,role,password:hashedPassword,email,avata})
