...
router.post('/addproduct', async function(req,res, next){
  try{
    const {name,price,quantity,id_cate,img,short_description,description} = req.body;
  const {_id} = new mongoose.Types.ObjectId()
  const data = await modelsproduct.create({_id,name,price,quantity,id_cate,img,short_description,description});
...
router.put('/editpro/:id', async function(req,res,next){
  try {
    const {id} = req.params;
    const{name,price,quantity,id_cate,img,short_description,description} = req.body;
    const data = await modelsproduct.findByIdAndUpdate(id,{name,price,quantity,id_cate,img,short_description,description});
