...
router.post('/addorder', async function(req,res,next){
    try {
        ...
        const {date,address,status,name,phonenumber,id_account,id_product} = req.body;
        const data = await modelsorder.create({_id,date,address,status,name,phonenumber,id_account,id_product})
        res.json(data)
    } catch (error) {
        res.json({mesage:'lỗi'})
    }
